...
{
    class EducationMap : EntityTypeConfiguration<Education>
    {
        public EducationMap()
        {
            HasKey(x => x.Id);
            ...
            HasRequired(x => x.People).WithMany(x => x.Educations).HasForeignKey(x => x.PeopleId).WillCascadeOnDelete(false);
